...
export class FilterUnitsService {
    ...
    for(let i = 0; i < unit.schedules.length; i++) {
      ...
      if (today_weekday === schedule_weekday) {
        if (schedule_hour !== 'Fechada') {
          let [unit_open_hour, unit_closed_hour] = schedule_hour.split(' às ');

          let unit_open_hour_int = parseInt(unit_open_hour.replace('h', ''), 10);
          let unit_closed_hour_int = parseInt(unit_closed_hour.replace('h', ''), 10);

          if (unit_open_hour_int <= open_hour_filter && unit_closed_hour_int >= close_hour_filter) {
            return true;
          } else {
            return false;
          }
  ...
  filter (results: Locations[], showClosed: boolean, hour: string) {
    ...
    // Filtra unidades abertas e fechadas
    if (!showClosed) {
      intermediateResults = results = results.filter(local => local.opened === true);
    }
